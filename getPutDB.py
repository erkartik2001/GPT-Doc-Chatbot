...
def createuser(uniqueid,name,role,upload=False):
    connection = connect_db()
    cursor = connection.cursor()

    create_query = """INSERT INTO userData (uuid,name,role,upload)
                        VALUES (%s,%s,%s,%s)
                   """
    
    cursor.execute(create_query,(uniqueid,name,role,upload))
    connection.commit()
